...
Dépôt **statique** (JSON + images) pour le portfolio de **Leo Giraud** — artiste 3D. Consommé par [`leo-giraud-site`](../leo-giraud-site).
...
3. Dans `leo-giraud-site`, copier `.env.example` → `.env.local` et mettre les URLs **raw** vers ce dépôt, par ex.  
